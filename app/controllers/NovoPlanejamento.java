	...
	@Entity
	public class NovoPlanejamento extends Model{ 

		/**
	     * Construtor da classe NovoPlanejamento, criando o gerenciador do sistema de Planejamento de Curso.
	     * @param curriculo NovaGrade utilizado no planejamento do curso do usuario.
	     */
	    ...
	    private NovaGrade curriculo;
	    private NovoPlanejamento sistemaPlanejamento;
		...
		public static Model.Finder<Long, NovoPlanejamento> find = new Model.Finder<Long, NovoPlanejamento>(
				Long.class, NovoPlanejamento.class);
	    /**
	     * Construtor da classe NovoPlanejamento, criando o gerenciador do sistema de Planejamento de Curso.
	     * @param curriculo Curriculo utilizado no planejamento do curso do usuario.
	     */
		public NovoPlanejamento(NovaGrade curriculo) {
		...
		public static void create(NovoPlanejamento sistemaPlanejamento) {
		...
		/**
		 * Adiciona o nono periodo na lista de Periodos.
		 */
		// INFORMATION EXPERT: Tem a lista de periodos.
			...
			this.periodo(periodo).removeDisciplina(disciplina);
				for (int i = periodo; i < getPeriodos().size(); i++) {
					for (int j = 0; j < periodo(i).getListaDeDisciplinas().size(); j++) {
						for (int k = 0; k < periodo(i).getListaDeDisciplinas()
								.get(j).getPreRequisitos().length; k++) {
							if (periodo(i).getListaDeDisciplinas().get(j)
									.getPreRequisitos()[k].equals(disciplina)) {
								removeDisciplinaDoPeriodo(i, periodo(i)
										.getListaDeDisciplinas().get(j)
										.getNomeDaDisciplina());
							}
						}
					}
